...
def collect_single_arm_noisy(
    ...
    tmp_dir = os.path.join(save_dir, f"tmp_single_arm_noisy_{str(time.time()).replace('.', '_')}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Collect noisy single-arm expert demonstrations in official robosuite HDF5 format.'
    )
    parser.add_argument('--num_episodes', type=int, default=50)
    parser.add_argument('--max_steps', type=int, default=400)
    parser.add_argument('--save_dir', type=str, default='./demonstrations/single_arm/noisy')
